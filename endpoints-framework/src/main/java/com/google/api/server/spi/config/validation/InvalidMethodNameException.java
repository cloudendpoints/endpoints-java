...
public class InvalidMethodNameException extends ApiMethodConfigInvalidException {
  public InvalidMethodNameException(ApiMethodConfig config, String name) {
    super(config, getErrorMessage(name));
  }

  private static String getErrorMessage(String name) {
    return String.format(
        "Invalid method name '%s'. The method name must match '\\w+(\\.\\w+)*'", name);
  }
}
