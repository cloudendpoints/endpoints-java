...
public class ApiConfigInvalidException extends ApiConfigException {
  public ApiConfigInvalidException(ApiConfig config, String message) {
    super(getErrorMessage(config, message));
  }

  private static String getErrorMessage(ApiConfig config, String message) {
    return String.format("%s: %s", config.getName(), message);
  }
}
