...
public class ApiClassConfigInvalidException extends ApiConfigException {
  public ApiClassConfigInvalidException(ApiClassConfig config, String message) {
    super(getErrorMessage(config, message));
  }

  private static String getErrorMessage(ApiClassConfig config, String message) {
    return String.format("%s.%s: %s", config.getApiConfig().getName(), config.getApiClassJavaName(),
        message);
  }
}
